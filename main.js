...
function showArrayOfTelNumbersBalanceMore2000$(arr) {
  let result = [];

  for (let user of arr) {
    if (user["balance"].slice(1).replaceAll(",",'') > 2000) {
      result.push(user["phone"]);
    }
  }
  console.log(result);     //  оскільки в задачі ВИВЕСТИ МАСИВ, то пишу console.log
  // return result;        //  якби було ПОВЕРНУТИ МАСИВ, то написав би return
}
showArrayOfTelNumbersBalanceMore2000$(users);


// Знайти суму всіх балансів користувачів

function getSumUsersBalance(arr) {
  let sumOfBalance = 0;

  for (let user of arr) {
    sumOfBalance += +user["balance"].slice(1).replaceAll(",",'');
  }
  return +sumOfBalance.toFixed(2);     //  повертаю суму всіх балансів користувачів
}
console.log(getSumUsersBalance(users));